  ...
  return (
    ...
    >
      ...
      <div className="p-3 sm:p-4">
        ...
        <h3 className="font-display font-bold text-sm sm:text-base text-[#1C1C1C] group-hover:text-emerald-600 transition-colors leading-tight line-clamp-2">
          {product.name}
        </h3>
        <div className="flex items-baseline gap-1.5 sm:gap-2 mt-1.5 sm:mt-2 flex-wrap">
          <span className="font-body font-bold text-emerald-600 text-sm sm:text-base">{formatPrice(product.price)}</span>
          {hasDiscount && (
            <span className="text-[11px] sm:text-sm text-[#6B6B6B] line-through">{formatPrice(product.compare_at_price)}</span>
          )}
        </div>
      </div>
    </Link>
