    ...
    return (
      <div className="max-w-lg mx-auto px-4 lg:px-8 py-16 text-center">
        <div className="animate-pulse space-y-6">
          <div className="w-20 h-20 rounded-full bg-cream-200 dark:bg-gray-700 mx-auto" />
          <div className="h-6 bg-cream-200 dark:bg-gray-700 rounded w-1/2 mx-auto" />
          <div className="h-4 bg-cream-200 dark:bg-gray-700 rounded w-1/3 mx-auto" />
          <div className="card p-6 space-y-3">
            <div className="h-4 bg-cream-200 dark:bg-gray-700 rounded w-3/4" />
            <div className="h-4 bg-cream-200 dark:bg-gray-700 rounded w-1/2" />
  ...
  if (error) {
    return (
      <div className="max-w-lg mx-auto px-4 lg:px-8 py-16 text-center">
        ...
        <h1 className="text-2xl font-display font-semibold text-[#1C1C1C] dark:text-gray-200">Payment Successful!</h1>
        <p className="text-[#6B6B6B] dark:text-gray-400 font-body mt-2">Your payment was successful.</p>
        <p className="text-sm text-[#6B6B6B] dark:text-gray-400 font-body mt-4 max-w-sm mx-auto">{error}</p>
  ...
  return (
    <div className="max-w-lg mx-auto px-4 lg:px-8 py-16">
      <div className="text-center">
        ...
        <p className="text-[#6B6B6B] dark:text-gray-400 font-body mt-2">Thank you for your purchase.</p>
      </div>

      <div className="card p-6 mt-8 space-y-4">
        <div className="flex justify-between items-center">
          <span className="text-sm text-[#6B6B6B] dark:text-gray-400">Order Number</span>
          <span className="font-mono font-bold text-[#1C1C1C] dark:text-gray-200">{order.order_number}</span>
        </div>
        ...
        {order.items && order.items.length > 0 && (
          <>
            ...
            <div>
              <p className="text-xs text-[#6B6B6B] dark:text-gray-400 mb-2 font-medium uppercase tracking-wider">Items</p>
              <div className="space-y-2">
                {order.items.map((item, i) => (
                  <div key={i} className="flex justify-between text-sm">
                    <span className="text-[#1C1C1C] dark:text-gray-200">
                      {item.name}
                      {item.size && <span className="text-[#6B6B6B] dark:text-gray-400"> — {item.size}</span>}
                      <span className="text-[#6B6B6B] dark:text-gray-400"> × {item.qty}</span>
                    </span>
        ...
        <p className="text-center text-sm text-[#6B6B6B] dark:text-gray-400 font-body mt-6">
