    ...
    return (
      <div className="max-w-3xl mx-auto px-4 lg:px-8 py-16 text-center">
        <div className="w-20 h-20 rounded-full bg-cream-200 flex items-center justify-center mx-auto mb-6">
          <HiOutlineShoppingBag className="w-10 h-10 text-[#6B6B6B]" />
        </div>
        <h1 className="text-2xl font-display font-semibold text-emerald-600">Your Cart is Empty</h1>
        <p className="text-[#6B6B6B] font-body mt-2">Looks like you haven't added anything yet.</p>
    ...
    <div className="max-w-5xl mx-auto px-4 lg:px-8 py-8">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-white rounded-2xl p-4 mb-4 border border-cream-200">
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs text-[#6B6B6B] font-body whitespace-nowrap">
          ...
          {items.map((item) => (
            <div key={item.key} className="card p-4 flex gap-4">
              <Link to={`/product/${item.slug}`} className="w-24 h-24 lg:w-28 lg:h-28 rounded-xl overflow-hidden bg-cream-200 flex-shrink-0">
                <img src={item.image} alt={item.name} className="w-full h-full object-cover" />
              </Link>
              <div className="flex-1 min-w-0">
                <Link to={`/product/${item.slug}`}>
                  <h3 className="font-body font-medium text-[#1C1C1C] hover:text-emerald-600 transition-colors line-clamp-1">
                    {item.name}
                  </h3>
                </Link>
                ...
                <div className="flex items-center justify-between mt-3">
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="w-8 h-8 rounded-lg border border-cream-300 flex items-center justify-center text-sm hover:bg-cream-100"
                    >
                      -
                    </button>
        ...
        <div>
          <div className="card p-6 sticky top-24">
            ...
            <Link to="/shop" className="block text-center text-sm text-[#6B6B6B] hover:text-emerald-600 mt-3 font-body transition-colors">
              Continue Shopping
            </Link>
          </div>
        </div>
